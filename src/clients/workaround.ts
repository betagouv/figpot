...
export async function getJsonResponseBody(response: Response): Promise<any> {
  ...
  if (response.status >= 200 && response.status < 300 && response.body && response.url.endsWith('api/main/methods/update-file')) {
  ...
  if (
    ...
    ((contentLength && parseInt(contentLength, 10) >= 0x1fffffe8) ||
      ...
      response.url.endsWith('api/main/methods/get-file')) // For Penpot to get an entire file (stricter condition since that's the prefix of other endpoints)
  ) {
